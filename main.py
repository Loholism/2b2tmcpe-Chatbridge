...
async def on_message(ctx):
  if ctx.channel.id==915274436347826227:
    if ctx.author.id==700275401314009168:
      channel=b.get_channel(961337454714060850)
      ...
      async with ClientSession() as session:
        ...
        await webhook.send(content=str(ctx.content).replace("@here", "@heree").replace("@everyone", "@eveyone"), username=ctx.author.name, avatar_url=ctx.author.avatar_url)
